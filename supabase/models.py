...
class CompanySourceIssues(Base):
    ...
    last_error = Column(Text)  # The most recent error message encountered when scraping this source
...
class JobScrapingIssues(Base):
    """Tracks scraping failures for individual jobs."""
    __tablename__ = 'job_scraping_issues'
    
    id = Column(Integer, primary_key=True)
    job_id = Column(Integer, ForeignKey('jobs.id', ondelete='CASCADE'), nullable=False, unique=True)
    failure_count = Column(Integer, nullable=False, default=0)
    last_failure = Column(DateTime(timezone=True), nullable=False, default=datetime.utcnow)
    last_error = Column(Text)
    created_at = Column(DateTime(timezone=True), nullable=False, default=datetime.utcnow)
    updated_at = Column(DateTime(timezone=True), nullable=False, default=datetime.utcnow)
    
    # Relationship
    job = relationship("Job", back_populates="scraping_issues")
...
class Job(Base):
    ...
    # New column
    needs_details = Column(Boolean, default=True)  # Flag to indicate if job needs detailed scraping
    
    # Relationships
    company = relationship('Company', back_populates='jobs')
    company_source = relationship('CompanySource', back_populates='jobs')
    scraping_issues = relationship('JobScrapingIssues', back_populates='job', uselist=False)
